    ...
    for (const req of requests) {
      ...
      const emitSubAgentStart = (sid, _itemIdx) => {
        ...
        events.push({
          ...
          timestamp: ts, // request start time (all parallel subagents share the same start)
          parentId: req.requestId,
          data: {
            subAgentId: sid,
      ...
      // Emit subagent.completed events — use request completedAt (all parallel subagents end together)
      for (const sid of seenSubAgents) {
        const agentName = subAgentNames[sid] || `subagent-${sid.slice(0, 8)}`;
        events.push({
          type: 'subagent.completed',
          id: `${req.requestId}-subagent-${sid}-end`,
          timestamp: completedAt, // request end time
          parentId: req.requestId,
          data: {
            toolCallId: sid,
            agentDisplayName: agentName,
